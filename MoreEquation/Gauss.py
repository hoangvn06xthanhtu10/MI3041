...
if __name__ == "__main__":
    print("--- CHƯƠNG TRÌNH GIẢI HỆ PT BẰNG GAUSS (SYMPY) ---")
    ...
    A_ref = A_aug.echelon_form()
    print("\n[2] Ma trận sau khi khử Gauss (Dạng bậc thang):")
    sp.pprint(A_ref)
    ...
    if solution == sp.EmptySet:
        ...
        for i in range(n):
            ...
            # Ép biểu thức về dạng tuyến tính 1D để đồng nhất định dạng phân số
            eq_str = f"x{i+1} = {sp.sstr(sol_list[i])}"
            
            # In thẳng ra màn hình rất gọn gàng
